...
export const ocean = {
  ...
  skyMid: colours.skyMid,
  seaTop: colours.seaTop,
  seaDeep: colours.seaDeep,
  card: colours.card,
...
export const BubbleButton: React.FC<{ title: string; onPress: () => void }> = ({ title, onPress }) => (
  <TouchableOpacity activeOpacity={0.9} onPress={onPress} style={{ marginTop: 8 }}>
    <LinearGradient
      colors={[ocean.accent, colours.accentAmber]}
      start={{ x: 0, y: 0 }}
      end={{ x: 1, y: 1 }}
      style={styles.btn}
    >
...
const styles = StyleSheet.create({
  btn: { paddingVertical: 14, paddingHorizontal: 22, borderRadius: 18, shadowColor: colours.black, shadowOpacity: 0.2, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 3 },
  btnText: { fontWeight: "800", fontSize: 16, textAlign: "center", color: colours.textOnYellowDark, letterSpacing: 0.5 },
  pill: { paddingHorizontal: 14, paddingVertical: 8, borderRadius: 999, backgroundColor: ocean.chip, borderWidth: 1, borderColor: colours.chipBorder },
  ...
  card: { backgroundColor: ocean.card, borderRadius: 20, padding: 14, shadowColor: colours.black, shadowOpacity: 0.08, shadowRadius: 12, shadowOffset: { width: 0, height: 4 }, elevation: 2, overflow: "hidden" },
});